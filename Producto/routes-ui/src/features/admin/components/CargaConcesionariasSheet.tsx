...
import { importarAutopistaJson } from "@/api/autopistas";
...
export function CargaConcesionariasSheet({ open, onOpenChange }: Props) {
  ...
  const uploadOne = async (item: FileItem) => {
    ...
    try {
      ...
      await importarAutopistaJson(json);
  ...
  const handleUpload = async () => {
    ...
    // Refresca el listado de concesionarias tras la tanda de subidas.
    ...
    <Sheet open={open} onOpenChange={(o) => (!o ? cerrar() : onOpenChange(true))}>
      <SheetContent className="w-full sm:max-w-lg flex flex-col gap-0 p-0">
        <SheetHeader className="border-b p-4">
          <SheetTitle>Importar concesionarias</SheetTitle>
          <SheetDescription>
            Sube uno o varios archivos <code className="font-mono">.json</code> con
            el formato de Autopista (metadatos + pórticos/tramos + tarifas).
          </SheetDescription>
        </SheetHeader>
